...
# (Day of week: [Start_Hour_1, Start_Hour_2, ...])
BOOKING_DAYS_HOURS = {'MONDAY': [15], 'FRIDAY': [15], 'SATURDAY': [10, 11], 'SUNDAY': [10, 11]}
DATE_STRFMT = '%m/%d/%Y'
...
for i in range(4):  # Bookings are available up to 3 days in advance.
    ...
    if day_of_week not in BOOKING_DAYS_HOURS:
        continue

    hours = BOOKING_DAYS_HOURS[day_of_week]
    try:
